...
*/
function private autoexec function_a0f244e()
{
	level notify(887832090);
}
...
/*
	...
	Flags: Linked
*/
function fall_and_bounce(killonimpact_speed, killonimpact_time, killonimpact)
{
	if(!isdefined(killonimpact))
	{
		killonimpact = 0;
	}
	self endon(#"death", #"change_state");
	...
	while(bouncedtime < maxbouncetime && lengthsquared(self.velocity) > sqr(10))
	{
		...
		{
			if(lengthsquared(impact_vel) > sqr(killonimpact_speed) || (util::timesince(fallstart) > killonimpact_time && lengthsquared(impact_vel) > (sqr(killonimpact_speed * 0.8))))
			{
				self kill();
			}
			else
