...
{
	self notify("31a77f79e352005f");
	self endon("31a77f79e352005f");
...
{
	self notify("6ebec6d613f789b4");
	self endon("6ebec6d613f789b4");
	while(true)
	{
		if(!isdefined(self))
		{
...
function function_4fd00e1f(localclientnum, oldval, newval, bnewent, binitialsnap, fieldname, bwastimejump)
{
	if(bwastimejump == 1)
	{
		...
		if(self.doa.var_10752c35 === 6)
		{
			...
			if(!isdefined(self.var_41b71195))
			{
				...
				{
					if(viewmodelhastag(fieldname, "tag_camera"))
					{
...
{
	self notify("99442c8747f9c87");
	self endon("99442c8747f9c87");
...
function function_7df2149d(localclientnum, orientation)
{
	...
	self notify("48cace66a8f8668a");
	self endon("48cace66a8f8668a");
	while(!isdefined(self.doa))
	{
		waitframe(1);
	}
