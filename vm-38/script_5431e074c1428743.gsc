...
function private function_c30fad9a()
{
	foreach(player in getplayers())
	{
		...
		/#
			...
			player.takedown.debug.var_8dbd3c70 = getdvarint(#"hash_576712f5a0d594b3", 0) || (getdvarint(#"debug_stealth", 0) ? 1 : undefined);
	...
	/#
		...
		while(true)
		{
			...
			foreach(corpse in corpses)
			{
				...
				print3d(var_2501cbe2, "", (1, 1, 1), 1, 0.25, 1, 1);
				line(corpse.origin, var_1157b993);
				print3d(var_1157b993, "", (1, 1, 1), 1, 0.25, 1, 1);
				print3d(corpse.origin + vectorscale((0, 0, 1), 10), "", (1, 1, 1), 1, 0.25, 1, 1);
				if(isdefined(corpse.found))
				{
					print3d(var_2501cbe2 + vectorscale((0, 0, 1), 20), "");
				}
				if(isdefined(level.stealth.corpse.var_4fc967d0))
				{
					foreach(guy in guys)
					{
						if(!isdefined(guy.stealth))
						...
						if(dist < sqr(level.stealth.corpse.var_4fc967d0[#"detect_dist"]))
						{
							line(guy.origin, var_2501cbe2, var_36abd3);
							continue;
						}
						if(dist < sqr(level.stealth.corpse.var_4fc967d0[#"sight_dist"]))
						{
