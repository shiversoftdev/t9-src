...
*/
function private autoexec function_d1b7dd0()
{
	level notify(2069814224);
}
...
function private function_b78294bf(victim, weapon, attackerweapon, var_67660cb2, time, meansofdeath)
{
	...
	if((isdefined(weapon.var_60a9eae7) ? weapon.var_60a9eae7 : 0) && isdefined(scoreevents.var_a6bfdc5f))
	{
		...
		self.multikills[attackerweapon.name].var_3e9f80b2++;
...
function function_5829abe3(attacker, weapon, var_651b6171)
{
	...
	if(var_651b6171.var_62c1bfaa === 1 || !is_true(var_629fbd5c) || is_true(destroyedkillstreak) && isdefined(var_3c727edf.var_3143c814) && util::function_fbce7263(attacker.team, self.team))
	{
...
function function_24d66e59(inflictor, meansofdeath, victim, attacker, weapon, var_bd10969, time, data)
{
	...
	foreach(effect in var_bd10969)
	{
		...
		if((isdefined(victim.var_60a9eae7) ? victim.var_60a9eae7 : 0) && (isdefined(scoreevents.var_a6bfdc5f) || isdefined(scoreevents.var_8600aca4)))
		{
			...
			attacker.multikills[effect.name].var_3e9f80b2++;
		...
		else if(isdefined(effect.var_4b22e697) && isplayer(effect.var_4b22e697) && attacker != effect.var_4b22e697 && attacker util::isenemyplayer(effect.var_4b22e697) == 0)
		{
			...
			if(isdefined(scoreevents.var_2eaed769) && (is_true(baseweapon.var_b76e0a09) || is_true(baseweapon.var_76ce72e8)))
			{
		...
		if(attacker == effect.var_4b22e697)
		{
			...
			function_4d412da8(inflictor, meansofdeath, victim, attacker, scoreevents, effect.var_3d1ed4bd, weapon, effect, time, data);
		}
	}
}
...
function function_f7f7b14e(data)
{
	...
	if(isarray(level.var_b13d1f9b))
	{
		foreach(var_25f92d1d in level.var_b13d1f9b)
		{
			if(isdefined(var_25f92d1d.kill_callback) && var_25f92d1d.var_358571b4 !== 1)
			{
				...
				attacker function_4d412da8(inflictor, meansofdeath, victim, attacker, var_b6f366b, var_a6f160ea, weapon, var_a6f160ea, time, data);
			}
		}
	}
	...
	if(isdefined(var_3d2a11cf.var_3143c814) && !is_true(var_629fbd5c) && (isdefined(attacker) && isdefined(victim) && util::function_fbce7263(attacker.team, victim.team)))
	{
	...
	baseweapon = weapons::getbaseweapon(weapon);
	attacker function_4d412da8(inflictor, meansofdeath, victim, attacker, function_3cbc4c6c(weapon.var_2e4a8800), weapon, weapon, baseweapon, time, data);
...
function function_706caaf3(meansofdeath, executioner, victim, var_e399c155, var_ff83a430)
{
	if(meansofdeath == "MOD_EXECUTION")
	{
		...
		executioner contracts::function_a54e2068(#"hash_ba2b17c3904e2d6");
		...
		executioner stats::function_dad108fa(#"hash_1a31e9546b33c22d", 1);
		executioner stats::function_dad108fa(#"hash_52b5cb629bbde814", 1);
		var_83a9a2ce = var_e399c155.type === #"melee";
		var_e605a21d = var_ff83a430.type === #"melee";
		if(var_83a9a2ce)
		{
		...
		if(var_83a9a2ce || var_e605a21d)
		{
			if(!isdefined(executioner.pers[#"hash_33dcd1468cbdb9c2"]))
			{
				executioner.pers[#"hash_33dcd1468cbdb9c2"] = 0;
			}
			executioner.pers[#"hash_33dcd1468cbdb9c2"]++;
			if(executioner.pers[#"hash_33dcd1468cbdb9c2"] == 2)
			{
				executioner stats::function_d0de7686(#"hash_2dc09a051d45038d", 1, #"hash_653135ec012bf674");
			}
		}
	}
}
...
function private function_d68ae402(inflictor, meansofdeath, victim, attacker, scoreevents, weapon, var_f801f37e, time)
{
	...
	if(attacker.multikills[var_ac4c1].kills >= 2)
	{
		...
		{
			attacker contracts::function_a54e2068(#"hash_7ce05509f5b3e35");
		}
	}
	...
	switch((isdefined(attacker.multikills[var_ac4c1].var_3e9f80b2) ? attacker.multikills[var_ac4c1].var_3e9f80b2 : 0))
	{
		case 0:
		case 1:
		case 2:
		...
		default:
		{
			if(attacker.multikills[var_ac4c1].var_3e9f80b2 > 2)
			{
	...
	attacker.multikills[var_ac4c1].var_3e9f80b2 = 0;
}
...
/*
	...
	Flags: Linked, Private
*/
function private function_4d412da8(inflictor, meansofdeath, victim, attacker, scoreevents, weapon, attackerweapon, var_f801f37e, time, data)
{
	...
	var_25f92d1d = level.var_b13d1f9b[var_f801f37e.name];
	if(isdefined(var_25f92d1d.kill_callback))
	{
		...
		{
			params = {#attackerweapon:attackerweapon, #attacker:attacker};
...
{
	if(scoreevents.var_4f57d8be === 1 && (sessionmodeismultiplayergame() || function_f99d2668()))
	{
...
{
	if(sessionmodeismultiplayergame() || function_f99d2668() && self.multikills[var_f801f37e.name].kills >= 2)
	{
...
function function_5a241bd8(weapon, callback, var_358571b4)
{
	...
	level.var_b13d1f9b[weapon.name].var_358571b4 = var_358571b4;
}
