...
function function_8b6ddd71(grenadeent, smokeweapon)
{
	...
	while(true)
	{
		...
		foreach(player in level.players)
		{
			...
			if(isdefined(smokeweapon.var_b999539c) && player istouching(smokeweapon.var_b999539c) && waitresult._notify == #"timeout")
			{
			...
			if(curval & mask)
			{
				...
				{
					trig = function_4cc4db89(var_ce7665b1, smokeweapon.var_b999539c);
				}
				if(!isdefined(trig))
				{
		...
		if(!isdefined(smokeweapon) || waitresult._notify != "timeout" || (!isdefined(smokeweapon.var_b999539c) && smokeweapon.item === getweapon(#"hash_531d2030796963b2")))
		{
			break;
		}
	}
