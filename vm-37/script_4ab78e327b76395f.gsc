...
function function_ebce8b6(a_ents)
{
	...
	if(player.var_9ebbaa46.victim !== player.takedown.body)
	{
		player.var_9ebbaa46.victim delete();
	}
	player.takedown.body show();
	player.takedown.body namespace_594b67e::function_b82cae8f(1);
	player namespace_594b67e::function_464d0412();
...
function body_drop(victim)
{
	...
	{
		if(self getplayerangles()[0] <= -15 && !self namespace_594b67e::function_fdff1cf3())
		{
			offset = vectorscale((0, 0, 1), 8);
			new_origin = playerphysicstrace(self.origin + offset, (self.origin + (anglestoforward(self.angles) * -20)) + offset);
			new_origin = playerphysicstrace(new_origin, new_origin - offset);
			self setorigin(new_origin);
			self val::set(#"hash_5d728b94d79614ca", "freezecontrols_allowlook", 1);
		}
		...
		self val::reset_all(#"hash_5d728b94d79614ca");
	}
