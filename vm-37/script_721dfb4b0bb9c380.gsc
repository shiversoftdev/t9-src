...
event main(eventstruct)
{
	...
	level.var_8b4ff11a = getgametypesetting(#"delayplayer");
		...
		if(var_1cfdf798)
		{
			...
			if(idamage.team == game.defenders && attacker.team == game.attackers)
			{
				...
				if(var_376742ed)
				{
					...
					scoreevents::function_2a2e1723(#"war_killed_defender_in_zone", attacker, idamage, weapon);
				}
			}
			if(var_376742ed)
			{
...
function on_player_killed(einflictor, attacker, idamage, smeansofdeath, weapon, vdir, shitloc, psoffsettime, deathanimduration)
{
	...
	{
		if(isdefined(psoffsettime.pers) && isdefined(psoffsettime.pers[#"hash_281165a5744a0185"]) && isdefined(psoffsettime.pers[#"hash_a8c7a84a8f7e333"]))
		{
...
function private on_zone_capture(sentient)
{
	...
	if(!isdefined(self.lastcaptureteam) || self.lastcaptureteam != capture_team)
	{
		...
		{
			var_1dbb2b2b = arraycopy(self.users[capture_team].var_5b307a20);
	...
	if(level.zones.size == level.capturedzones)
	{
		round::function_d1e740f6(game.attackers);
		level thread globallogic::end_round(1);
	}
	else
	{
...
function private credit_player(player, string, capturetime, capture_team, lastcaptureteam, var_a84f97bf, var_af8f6146)
{
	...
	if(!is_score_boosting(string))
	{
		...
		if(isdefined(string.pers[#"captures"]))
		{
			...
			{
				string contracts::function_a54e2068(#"hash_47886fea64d2a151");
			}
		}
		...
		if(var_af8f6146)
		{
			...
			string contracts::function_a54e2068(#"hash_4fa0008b60deaab4");
...
/*
	...
	Flags: Private
*/
function private function_40843d72(team)
{
	if(team != "all")
	{
		return;
...
{
	self notify("4749dd2e068e413");
	self endon("4749dd2e068e413");
