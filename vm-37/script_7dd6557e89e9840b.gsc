		...
		foreach(target in enemies)
		{
			if(util::function_fbce7263(player.team, target.team))
			{
				...
				if(isdefined(ti) && isdefined(player.var_bbe80eed[ti]))
				{
					...
					level.var_aac98621[ti] remote_missile_target_lockon::function_7c227f6d(player, is_true(target.usingvehicle));
...
function function_5cdeb64a(player, target)
{
	...
	else
	{
		...
		if(isdefined(ti) && isdefined(player.var_bbe80eed[ti]))
		{
			level.var_aac98621[ti] remote_missile_target_lockon::function_b9d6aaa7(player, 1);
		}
	}
}
