...
function function_d5766919(killstreaktype)
{
	if(isplayer(self))
	{
		...
		if(var_1120bf0.var_bd027dd9 != 32767)
		{
			...
			{
				var_d90e0e15.var_8e092725 = 0;
				self namespace_b376ff3f::function_fba40e6c(var_d90e0e15);
			}
		}
		else
		{
...
function actor_damage_override(inflictor, attacker, damage, flags, meansofdeath, weapon, var_fd90b0bb, vpoint, vdir, shitloc, psoffsettime, boneindex, surfacetype)
{
	...
	element = namespace_42457a0::function_d6863a3(inflictor, attacker, meansofdeath, weapon, aat);
	...
	if(isplayer(attacker) && is_true(attacker.var_d26d670e))
	{
		var_c55e30bd = int(0.05 * final_damage);
		attacker.health = attacker.health + var_c55e30bd;
		if(attacker.health > attacker.var_66cb03ad)
		{
			attacker.health = attacker.var_66cb03ad;
		}
	}
	if(isplayer(attacker))
	{
...
function actor_damage_override_wrapper(inflictor, attacker, damage, flags, meansofdeath, weapon, var_fd90b0bb, vpoint, vdir, shitloc, vdamageorigin, psoffsettime, boneindex, modelindex, surfacetype, vsurfacenormal)
{
	...
	else
	{
		...
		if(isdefined(self.var_94378ef) && (self.health - damage_override) <= self.var_94378ef && !is_true(self.marked_for_death))
		{
	...
	if(!willbekilled || !is_true(self.dont_die_on_me))
	{
		...
		if(!zm_utility::is_survival() && !isdefined(level.var_f96e7a56[weapon]))
		{
			...
			element = self namespace_42457a0::function_d6863a3(inflictor, attacker, meansofdeath, weapon, aat);
		...
		self finishactordamage(inflictor, attacker, damage_override, flags, meansofdeath, weapon, var_fd90b0bb, vpoint, vdir, shitloc, vdamageorigin, psoffsettime, boneindex, surfacetype, vsurfacenormal);
		if(isplayer(attacker) && damage_override > 0 && !is_true(self.var_265cb589) && !is_true(level.var_dc60105c) && (!attacker issplitscreen() || getplayers().size < 4))
		{
...
function end_game()
{
	...
	if(zm_utility::is_survival())
	{
		...
		level.gameended = 1;
		function_2a49523d(level.winningteam);
...
function function_d3113f01(var_bfe774a8)
{
	...
	if(!isdefined(level.var_4b2568ad) || level.var_4b2568ad.round != round_number)
	{
		...
		level.var_4b2568ad = {#parameters:parameters, #round:round_number};
	}
...
/*
	...
	Flags: None
*/
function function_78e7b549(score)
{
	if(scoreevents::shouldaddrankxp(self))
	{
		self.pers[#"totalmatchbonus"] = score;
		self addrankxpvalue("match_end_xp", score, 3);
		self.matchbonus = score;
	}
}
...
{
	if(self isswitchingweapons() || isdefined(self.laststandpistol) || self zm_utility::is_drinking() || is_true(self.var_bd5b6650))
	{
		return 0;
	}
