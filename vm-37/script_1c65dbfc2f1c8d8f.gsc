...
{
	self notify("79355800756fc059");
	self endon("79355800756fc059");
	/#
		assert(isplayer(self));
	#/
...
{
	self notify("50908adf85aee4e7");
	self endon("50908adf85aee4e7");
	/#
		assert(isplayer(self));
	#/
	while(isdefined(self) && is_true(self.var_10abd91d))
...
{
	self notify("34f671be1dd2fbfa");
	self endon("34f671be1dd2fbfa");
	/#
		assert(isplayer(self));
	#/
	if(!isdefined(self))
...
{
	self notify("5a84c77a020df3bb");
	self endon("5a84c77a020df3bb");
	/#
		assert(isplayer(self));
	#/
	if(!isdefined(self))
...
function function_5852cb7b(var_bd027dd9, var_dfe6c7e5, var_b44c0e7e, var_cda2ff12)
{
	...
	for(i = 0; i < self.inventory.items.size; i++)
	{
		if(self.inventory.items[i].var_bd027dd9 === var_bd027dd9)
		{
			...
			{
				if(isdefined(array::find(array(17 + 1, ((17 + 1) + 8) + 1), i)) && function_57eac2ca() == 0)
				{
...
function function_73ae3380(item, var_a8bccf69)
{
	...
	if(isdefined(level.var_fee1eaaf) && var_a8bccf69 < 2)
	{
		...
		if(isdefined(var_e9c4fe29))
		{
			...
			while(currentweapon === level.weaponnone)
			{
				waitframe(1);
				currentweapon = self get_current_weapon();
			}
